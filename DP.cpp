...
int calculateAdditionalLength(double targetPercentage, int tireLength) const {
    double targetSpace = (targetPercentage / 100.0) * (rows * cols);
    double remainingEmptySpace = max(targetSpace - filledSpace, 0.0);
    int additionalTires = max(static_cast<int>(remainingEmptySpace / (tireLength * tireLength + 0.0)), 0);
...
int main() {
    ...
    cout << "Enter the number of length and breadth of the container: ";
   ...
   // Check if the container is filled below 80% and recommend additional lengths
    double targetPercentage = 90.0;
    for (const Tire& tire : tires) {
        if (container.getPercentageFilled() < targetPercentage) {
        int additionalTires = container.calculateAdditionalLength(targetPercentage, tire.length);
        cout << "Recommended additional tires of length " << tire.length << ": " << additionalTires << endl;
    } else {
        break; // Stop recommending once the target percentage is reached
    }
}
