...
class GUI:
    ...
    def generate_tire_combinations(self, all_tires):
        ...
        for r in range(1, min(len(all_tires), 4) + 1):
    ...
    def setup(self):
        ...
        ttk.Label(self.button_frame, text="Enter the length of the container:", font=("Arial", 12)).grid(row=0, column=0, padx=5, pady=5)
        self.rows_entry = ttk.Entry(self.button_frame)
        self.rows_entry.grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(self.button_frame, text="Enter the breadth of the container:", font=("Arial", 12)).grid(row=1, column=0, padx=5, pady=5)
        self.cols_entry = ttk.Entry(self.button_frame)
        self.cols_entry.grid(row=1, column=1, padx=5, pady=5)

        ttk.Label(self.button_frame, text="Enter the number of tires:", font=("Arial", 12)).grid(row=2, column=0, padx=5, pady=5)
    ...
    def save_to_pdf(self, container, additional_tires_recommendations, pdf_filename):
        ...
        pdf.drawString(50, 730, f"Length of Container: {container.rows} ft")
        pdf.drawString(50, 710, f"Breadth of Container: {container.cols} ft")
        pdf.drawString(50, 690, f"The Percentage of container filled: {container.get_percentage_filled():.2f}%")

        pdf.setFont("Helvetica-Bold", 16)  # Set font to bold and adjust font size as needed
        pdf.drawString(50, 550, "Visualization of Tire Placement in the Container")
        ...
        y_position = 750
        color_mapping = {"A": "Red", "B": "Green", "C": "Blue", "D": "Yellow", "E": "Purple", "F": "Pink"}
        for i, tire in enumerate(self.tires):

            y_position -= 20
            color_name = color_mapping.get(tire.color, "Unknown")  # Get color name from mapping
            pdf.drawString(50, y_position, f"Tire {i + 1} - Length: {tire.length}, Symbol: {tire.symbol}, Color: {color_name}")
        ...
        for length, count in additional_tires_recommendations.items():
            ...
            pdf.drawString(50, y_position, f"Tire Length: {length}, Recommendation: {count}, Percent Filled after Recommendation: {filled_after_recommendations:.2f}%")
            y_position -= 20
